...
from filmy.data_herci import herci_data
...
def seznam_hercu(request):
    return render(request, 'filmy/seznam_herci.html', context={'herci': herci_data})

def detail_herce(request, slug):
    if slug in herci_data:
        herec = herci_data[slug]
        return render(request, 'filmy/detail_herci.html', context={'herec' : herec})
    else:
        raise Http404 ("Herec nebyl nalezen")
